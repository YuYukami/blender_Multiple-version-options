...
class BlenderPlugin(DeadlinePlugin):
    ...
    def RenderExecutable(self):
        ...
        # 嘗試獲取特定版本的 Blender 可執行文件列表
        try:
            executableList = self.GetConfigEntry("Blender_%s_RenderExecutable" % blVersion)
            self.LogInfo("Executable List: %s" % executableList)
        except KeyError:
            # 如果找不到特定版本的配置，設置默認可執行文件列表
            executableList = self.GetConfigEntry("Blender_RenderExecutable")
            self.LogWarning("Could not find specific Blender version executable. Using default executable list.")
        
        # 確保 executable 變量被正確賦值
        executable = ""
        if executableList:
            if build == "32bit":
                ...
                if executable == "":
                    self.LogWarning("32 bit Blender render executable was not found in the semicolon separated list \"%s\". Checking for any executable that exists instead." % executableList)
            
            elif build == "64bit":
                self.LogInfo("Enforcing 64 bit build of Blender")
                executable = FileUtils.SearchFileListFor64Bit(executableList)
                if executable == "":
                    self.LogWarning("64 bit Blender render executable was not found in the semicolon separated list \"%s\". Checking for any executable that exists instead." % executableList)
            
            if executable == "":
                self.LogInfo("Not enforcing a build of Blender")
                executable = FileUtils.SearchFileList(executableList)
                if executable == "":
                    self.FailRender("Blender render executable was not found in the semicolon separated list \"%s\". The path to the render executable can be configured from the Plugin Configuration in the Deadline Monitor.")
        else:
            self.FailRender("No Blender executable was found.")
